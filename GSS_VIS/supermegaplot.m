function supermegaplot(t, y, yfil, opts)
%SUPERMEGAPLOT
...
optV = contains(opts,'V');
optF = contains(opts,'F');
...
if contains(opts,'v') || contains(opts,'r') || contains(opts,'h')
    v = mod(v,2*pi);
%     while sum(v(:)>2*pi) ~= 0
...
if contains(opts,'r') || contains(opts,'h')
