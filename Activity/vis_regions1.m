function [ H, latsat, area ] = vis_regions1( CART510, jd )
...
COORD6 = CART510([211 242 271 300 270 240],1:3);

apexbetha = []; apextheta = [];

for i = [211 242 271 300 270 240]
    x2 = CART510(i,1);
    y2 = CART510(i,2);
    z2 = CART510(i,3);
    ...
    bethas = asin(z/R0)/deg;
    thetas = atan2(y,x)/deg;
    ...
    XYZ = R3(pi/2-lon2*pi/180)*R1(pi/2-lat2*pi/180)*GG;
...
% areatri = 4*pi*R0^2 * (areaint(apexbetha([1 2 3]),apextheta([1 2 3])) + ...
%    areaint(apexbetha([4 5 6]),apextheta([4 5 6])) + ...
%    areaint(apexbetha([1 3 6]),apextheta([1 3 6])) + ...
%    areaint(apexbetha([3 4 6]),apextheta([3 4 6])));
%area = areatri;
...
GG = zeros(3,length(t));
GG(1,:) = xg; GG(2,:) = yg; GG(3,:) = zg;
XYZ = R3(pi/2-lon1*pi/180)*R1(pi/2-lat1*pi/180)*GG;
