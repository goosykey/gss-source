...
orb = [an-Da en 0 10.9*deg in-Di*deg pi/4 m];
y00 = math.orbitize(orb);
...
while ~converged && stepno < maxstep
    ...
    fprintf('current XSI = %6.4f deg \n', XSI / deg);
    fprintf('--------------------\n');
    fprintf('last counted duration %4.2f days; \n', t0/86400);
    fprintf('last counted residual "di" = %6.4f deg \n', di / deg);
    %fprintf('ink = %4.2f; in = %4.2f \n', ink/deg, in/deg);
    tic;
    
    SATDATA{3} = ones(360,1)*XSI; 
    ...
    while ak < an
        ...
        [ti, yi] = ode45('earth353_FULL', jd0s+t0:h:jd0s+t0+T, y0, options);
