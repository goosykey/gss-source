...
elseif nargin == 6
    U = 398600.4418; %Gravitational Constant for Earth (km^3/s^2)
end
...
for j = 1:length(a)
    ...
    R(:,j) = math.R3(-Omega(j))*math.R1(-inc(j))*math.R3(-w(j))*R_pqw;
    V(:,j) = math.R3(-Omega(j))*math.R1(-inc(j))*math.R3(-w(j))*V_pqw;
end